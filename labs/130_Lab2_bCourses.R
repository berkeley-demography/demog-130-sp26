...
########################################
# Question 1 Mortality Rates by Age
########################################
...
# Read CSV files for students who need it
# usa_m <- read.csv("usa_m.csv", header = TRUE)
...
########################################
  ...
  # Generate a new data frame that is keeps Year, Age, ax
  usa_nax <- select(usa_hmdlt, Year, Age, ax)
  # For legibility below, rename ax nax
  usa_nax$nax <- usa_nax$ax
  
  
  # Keep the years 1933, 1950, 1965, 1980, 1995, 2005, 2019 for the nax data frame 
  ...
  # Generate Lx as the sum of (survivors x interval width) + (deaths x PY lived by people dying in the interval)
  ...
  # How could we check if we've done this correctly? 
